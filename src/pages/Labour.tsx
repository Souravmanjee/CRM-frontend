...
import { Plus, Loader2, X, Edit2, Trash2, Users } from 'lucide-react';
import type { LabourEntry as LabourLog } from '../store/labourStore';
import { ExportButton } from '../components/common/ExportButton';
import type { ColumnDef } from '../components/common/ExportButton';

const LabourModal = ({ onClose, entry }: { onClose: () => void, entry?: LabourLog }) => {
    const addLog = useLabourStore(s => s.addLabour);
    const updateLog = useLabourStore(s => s.updateLabour);
    const isLoading = useLabourStore(s => s.isLoading);
    const activeBusiness = useBusinessStore(s => s.activeBusiness);

    const [form, setForm] = useState({
        date: entry?.date ? new Date(entry.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        name: entry?.staffName || '',
        payment: entry?.amount?.toString() || '',
        remark: entry?.remark || '',
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const data: Partial<LabourLog> = {
                businessId: activeBusiness?._id,
                date: form.date,
                staffName: form.name.trim(),
                amount: parseFloat(form.payment) || 0,
            ...
            setError(err.message || `Failed to ${entry ? 'update' : 'add'} labour entry`);
        }
    };
            ...
            <div className="bg-white dark:bg-bg-dark-secondary rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-border-dark sticky top-0 bg-white dark:bg-bg-dark-secondary z-10">
                    <h2 className="text-xl font-heading font-bold flex items-center gap-2">
                        ...
                        {entry ? 'Edit Labour Entry' : 'New Labour Entry'}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Worker Name *</label>
                            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="input-field w-full" placeholder="e.g. Suresh" required />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Payment (₹) *</label>
                            <input value={form.payment} onChange={e => setForm({ ...form, payment: e.target.value })} className="input-field w-full" type="number" step="0.01" min="0" placeholder="0.00" required />
                        </div>
                        <div>
...
export const Labour = () => {
    const { labours: logs, isLoading, fetchLabours: fetchLogs, deleteLabour: deleteLog } = useLabourStore();
    ...
    const handleDelete = async (id: string, name: string, date: string) => {
        if (window.confirm(`Delete payment record for ${name} on ${new Date(date).toLocaleDateString()}?`)) {
            await deleteLog(id);
        }
    };
    ...
    const exportColumns: ColumnDef<LabourLog>[] = [
        { header: 'Date', key: (r) => new Date(r.date).toLocaleDateString('en-IN') },
        { header: 'Worker Name', key: 'staffName' },
        { header: 'Remarks', key: 'remark' },
        { header: 'Payment', key: 'amount' }
    ];
            ...
            <div className="flex flex-col h-full">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Track daily wages and labour payments.</p>
                    </div>
                    <div className="flex gap-2">
                        <ExportButton data={logs} columns={exportColumns} filename="Labour_Logs" />
                ...
                <div className="card flex flex-col flex-1 overflow-hidden">
                    <div className="overflow-x-auto">
                        {isLoading && logs.length === 0 ? (
                            ...
                            <div className="p-12 text-center text-slate-500">No labour records found.</div>
                        ) : (
                            <table className="w-full text-left whitespace-nowrap text-sm">
                                <thead>
                                    <tr className="border-b border-slate-200 dark:border-border-dark uppercase tracking-wider text-slate-500 bg-slate-50 dark:bg-slate-800/20">
                                        ...
                                        <th className="px-6 py-4 font-semibold text-right text-blue-600 bg-blue-50/50 dark:bg-blue-900/10">Payment</th>
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                                    {logs.map(log => (
                                        <tr key={log._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 group">
                                            ...
                                            <td className="px-6 py-4 text-right font-bold text-blue-600 dark:text-blue-400 bg-blue-50/50 dark:bg-blue-900/10">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <button onClick={() => handleDelete(log._id, log.staffName, log.date)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            </td>
